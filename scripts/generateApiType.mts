...
await temporaryFileTask(async (tempFilePath) => {
  ...
  for (const currentPath in openApiJson.paths) {
    for (const currentOperationIndex in openApiJson.paths[currentPath]) {
      ...
      if (currentOperation?.operationId) {
        ...
        openApiJson.paths[currentPath][currentOperationIndex].operationId = currentOperationPartNames.join('_v1');
      }
    }
  }
  ...
  await $`npx openapicmd read --strip openapi_client_axios --format json src/api/openapi.json > src/api/openapi-runtime.json`;
});
