...
class Transfert implements TransfertInterface {
    ...
    async downloadAndReadFile(remotePath: string): Promise<string> {
        try {
            await this.open();
            return await this.client.downloadAndReadFile(remotePath, Config.get().edi.fileEncoding as BufferEncoding);
        }
        // eslint-disable-next-line no-useless-catch
        catch (error) {
            throw error;
    ...
    async moveToErrorDir(sourcePath: string): Promise<void> {
        return this.moveFile(sourcePath, path.join(Config.get().edi.wmsPathErrorDir, path.basename(sourcePath)));
    }

    async moveToArchiveDir(sourcePath: string): Promise<void> {
        return this.moveFile(sourcePath, path.join(Config.get().edi.wmsPathArchiveDir, path.basename(sourcePath)));
    }
}
