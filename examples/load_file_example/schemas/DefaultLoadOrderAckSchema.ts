...
import {
  ...
  OrderItemInterface,
  OrderTypeEnum,
} from "../LoadOrderAcknowledgeTaskExample.mts";
  ...
  mapInputFileData(rawData: object[]): OrderInterface[] | ExitOrderInterface[] {
    ...
    rawData.forEach((line) => {
      ...
      const defautOrderItem: OrderItemInterface = {
        ...
        expected_quantity: parseInt(line["Quantite article"]),
        item_packaging_type: Config.get().edi.wmsItemPackagingType as ItemPackagingTypeEnum,
        serial_shipping_container_code:
          line["Code barre sur l'article"]?.length > 0 ? line["Code barre sur l'article"] : null,
    ...
    const executionDate = DateTime.fromFormat(firstItem["Date de mouvement"], "yyyyMMdd").toFormat(
